...
export default function AdminInventoryPage() {
  ...
  return (
    <div>
      <PageHeader title="إدارة المخزون (إدارة)" description="نفس بيانات المخزون الحقيقية المستخدمة في بوابة الصيدلاني" action={<StockFormDialog />} />

      {isLoading && <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div>}
      {!isLoading && (!items || items.length === 0) && <EmptyState icon={Boxes} title="لا توجد دفعات مخزون مسجّلة بعد" />}
